...
- **Текстуры блоков** из папки `assets/`: грузятся в массив текстур (не в атлас — у
  атласа на мип-уровнях подтекают соседние клетки). Одна картинка земли служит нескольким
  блокам: песок как есть, равнина под светло-салатовым фильтром, земля под коричневым, а
  снег делается из неё обесцвечиванием при загрузке. Вода полупрозрачная — сквозь неё
  видно дно. Если картинок нет, блоки рисуются цветом и игра всё равно запускается.
...
отдаёт свою карточку, её видно прямо в браузере. В игре достаточно вписать имя сервиса
(`имя.onrender.com`): https клиент открывает системными средствами — на Android через
`HttpsURLConnection`, в настольной сборке через OpenSSL.
