...
**Игра для Android.** Хардкорное выживание в открытом мире (механики Rust): процедурная
карта 4000×4000 метров, добыча, крафт, строительство, рейды, кланы и выделенный сервер
на 100+ игроков.
...
|---|---|---|
| 1 | Ядро движка, процедурный мир, биомы, ресурсы, монументы, сутки и погода, каркас сервера, **играбельный Android-клиент с сенсорным управлением** | ✅ готово |
...
Левая половина экрана — джойстик движения (появляется под пальцем), правая —
обзор перетаскиванием. Кнопки: **УДАР** (добыча), **E** (действие, напиться),
**ПРЫЖОК**, **БЕГ**, **СЕСТЬ**, сверху справа — **ИНВ**, **КРАФТ**, **КАРТА**.
Подробнее и почему сделано именно так — [docs/ANDROID.md](docs/ANDROID.md).
...
- **Android-клиент**: мир строится на устройстве из сида (0.15 с), потоковая подгрузка
  чанков рельефа вокруг игрока, деревья и камни из того же рассева, что у сервера,
  небо с солнцем, звёздами и облаками, туман по погоде, вода, сенсорное управление,
  HUD (здоровье, голод, жажда, силы, радиация), окна инвентаря, крафта и карты.
- **Ходьба, бег, присед, прыжок, плавание**, урон при падении, метаболизм по ТЗ
  (регенерация 1 HP/с при сытости и жажде выше 80), добыча ресурсов ударами.
