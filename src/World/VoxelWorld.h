...
public:
    ...
    // ---- Восстановление жил. Выбитая руда и камень возвращаются: без этого карту
    // вычищают за час и добывать становится нечего. Восстанавливается только то, что
...
private:
    ...
    // Выбитые блоки декора: ключ блока -> сколько секунд осталось и где он стоял.
    // Координаты храним рядом, а не распаковываем из ключа: упаковка сжимает их с
