...
enum class EventType : int {
    ...
    BagDrop,      // стак в мешке погибшего: id — мешок, c — ячейка, a — предмет, b — сколько
    BagTake,      // из мешка забрали ячейку: id — мешок, c — ячейка
    Shot,         // выстрел: a — чем стреляли (ItemType). Направление берётся из позы
                  // стрелка, она и так едет каждый обмен — второй раз слать её незачем.
    Throw,        // граната или ракета пошла: id — 0 граната, 1 ракета, xyz — откуда,
                  // a/b/c — скорость, умноженная на сто (в целых, как и всё здесь)
};
...
struct Event {
    ...
    // Третье число. Понадобилось мешку погибшего: в нём тридцать ячеек, и без номера
    // ячейки два одинаковых стака в одном мешке не различить.
    int   c = 0;
    float x = 0, y = 0, z = 0;
    long long seq = 0;        // номер в журнале сервера
};
