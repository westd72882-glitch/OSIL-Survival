...
// Вершина кубического мира: позиция, нормаль грани, цвет и координаты текстуры со
// слоем. Цвет здесь не «вместо текстуры», а фильтр поверх неё: в нём же едут затенение
// по углам и разброс яркости, поэтому один блок травы отличается от соседнего, хотя
// текстура у них общая.
struct VoxelVertex {
    float px, py, pz;
    float nx, ny, nz;
    float r, g, b;
